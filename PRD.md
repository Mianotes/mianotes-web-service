...
Mianotes is a family/shared local knowledge base. Once someone has access to the
local instance, they can browse all notes and topics. Users exist mainly for
ownership, attribution, filtering, and permissions.
...
Per-user API tokens are paused for the current v1 browser flow. Future token
rules:

- Tokens are generated by the app.
- Tokens are stored outside normal browser responses.
...
the auth endpoints. Raw developer tokens are out of scope for the current
browser flow and should not be casually exposed in normal browser responses when
