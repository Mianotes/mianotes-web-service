...
need the raw key to authenticate a request.

When you configure `MIANOTES_API_KEY` in the web service environment, the raw
secret lives in your environment. On startup and on authenticated requests,
Mianotes can sync the derived public verifier into the active database. This is
what lets the same service key work after a restart, and across database
switches, without storing the raw key in the database.
...
If you are scripting the whole flow, sign in with the API and save the session
cookie:
